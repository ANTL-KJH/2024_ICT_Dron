...
def switch3_callback(channel):
    print("스위치 3의 상태가 변경되었습니다.")
    # 여기에 스위치 2의 상태에 따른 작업을 추가하세요.

def switch4_callback(channel):
    print("스위치 4의 상태가 변경되었습니다.")
    # 여기에 스위치 2의 상태에 따른 작업을 추가하세요.

# 초기 스위치 상태가 OFF일 때만 인터럽트 이벤트 설정
GPIO.add_event_detect(switch1_pin, GPIO.FALLING, callback=switch1_callback, bouncetime=200)
GPIO.add_event_detect(switch2_pin, GPIO.FALLING, callback=switch2_callback, bouncetime=200)
GPIO.add_event_detect(switch3_pin, GPIO.FALLING, callback=switch3_callback, bouncetime=200)
GPIO.add_event_detect(switch4_pin, GPIO.FALLING, callback=switch4_callback, bouncetime=200)
